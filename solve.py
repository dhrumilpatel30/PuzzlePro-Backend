...
async def solver(file: UploadFile):
 cloudinary.config(
        cloud_name="dnnz1zpyw",
        api_key="621955915337499",
        api_secret="IVDyLX0emJ05EQjPffG3n691osk"
 )
 img_bytes = await file.read()
 img = cv2.imdecode(np.frombuffer(img_bytes, dtype=np.uint8), flags=cv2.IMREAD_COLOR)

 local_filename = "temp_image.png"
 cv2.imwrite(local_filename, img)
 upload_result = upload(local_filename, use_filename=True, unique_filename=True)
 img1 = upload_result['secure_url']
